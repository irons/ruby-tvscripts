...
# 
# This script was originally written to download meta data for your TV shows.  I have
# rewritten it to only be a fast file renamer.  It keeps a cache of all of your filenames
# checking them every 10 days to see if the filename has changed.  You can force a recheck
# of all filenames as well.
#
# The name format is configurable.  Settings can be saved in a ".ruby-tvrenamer" file which
# is a YML file with the following optional settings:
# 
# format
# 
...
class Series 
	...
	def initialize(name, refresh)
		...
		@name = name
		@episodes = Hash.new
#		@series_xml_path.delete if @series_xml_path.file? && refresh
#		@series_xml_path.delete if @series_xml_path.file? && File.new(@series_xml_path).ctime < (Time.now - @@series_cache)
#		@series_xml_path.delete if @series_xml_path.file? && rand > 0.9
#		@episodes_xml_path.delete if @episodes_xml_path.file? && refresh
#		@episodes_xml_path.delete if @episodes_xml_path.file? && File.new(@episodes_xml_path).ctime < (Time.now - @@series_cache)
#		@episodes_xml_path.delete if @episodes_xml_path.file? && rand > 0.9
...
# Renames the file to format specified in config
